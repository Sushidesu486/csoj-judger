...
from oj_checker.review_bundle import VerifiedReviewBundle, verify_review_bundle
...
class AgentRunError(RuntimeError):
    """A review run request or persistent record is invalid."""


class AgentRunQueueFull(AgentRunError):
    """The bounded in-memory dispatch queue has no free slot."""


class AgentRunFailure(RuntimeError):
    """A worker failed with a stable, externally safe error code."""

    def __init__(self, code: str) -> None:
        if not code or not re.fullmatch(r"[A-Z][A-Z0-9_]{1,63}", code):
            raise ValueError("agent run failure code is invalid")
        super().__init__(code)
        self.code = code
...
class FileAgentRunService:
    """A bounded, persistent single-node queue for the temporary Agent rollout.

    Requests and results are immutable create-only files. State changes are an
    append-only event log, so a process crash cannot turn a failed/incomplete
    review into a completed report. Retryable failures are reconciled in the
    same immutable run with a bounded attempt count.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if worker_count < 0 or worker_count > 16:
            raise ValueError("worker_count must be between zero and sixteen")
        if executor is None and worker_count:
            raise ValueError("worker_count requires an Agent executor")
        if max_queued <= 0:
        ...
        if not public_keys:
            raise ValueError("at least one review bundle public key is required")
        self._root = Path(root) / "agent-runs"
    ...
    def create(self, envelope: bytes) -> dict[str, Any]:
        now = self._aware_now()
        bundle = verify_review_bundle(envelope, self._public_keys, now=now)
        ...
        if not isinstance(submission, Mapping):
            raise AgentRunError("verified bundle submission disappeared")
        submission_id = submission.get("id")
        if not isinstance(submission_id, str):
            raise AgentRunError("verified bundle submission ID disappeared")
        run_id = f"review-{bundle.payload_digest}"
        with self._lock:
            if self._queue.full():
                raise AgentRunQueueFull("agent review queue is full")
    ...
    def get(self, run_id: str) -> dict[str, Any]:
        if _RUN_ID.fullmatch(run_id) is None:
            raise AgentRunError("invalid Agent run ID")
        with self._lock:
    ...
    def _read_run(self, run_root: Path) -> dict[str, Any]:
        ...
        if result_path.is_file() and not result_path.is_symlink():
            response["result"] = _read_json_object(result_path, max_bytes=2 << 20)
        return response

    def latest(self, submission_id: str) -> dict[str, Any] | None:
        return self.latest_many((submission_id,)).get(submission_id)

    def latest_many(self, submission_ids: Iterable[str]) -> dict[str, dict[str, Any]]:
        requested: set[str] = set()
        for submission_id in submission_ids:
            requested.add(_canonical_submission_id(submission_id))
        if not requested:
            return {}
        with self._lock:
            return {
    ...
    def _remember(self, run: dict[str, Any]) -> None:
        ...
        if not isinstance(run_id, str) or _RUN_ID.fullmatch(run_id) is None:
            raise AgentRunError("Agent run record has an invalid run ID")
        if not isinstance(submission_id, str) or not isinstance(created_at, str):
            raise AgentRunError("Agent run record has incomplete metadata")
    ...
    def _reconcile_loop(self) -> None:
        while not self._reconcile_stop.wait(self._reconcile_interval_seconds):
            try:
                ...
                _LOGGER.exception("Agent run reconciliation failed")
    ...
    def _recover_persisted_result(self, run_id: str) -> bool:
        ...
        if result_path.is_symlink() or not result_path.exists():
            return False
        try:
            _read_json_object(result_path, max_bytes=2 << 20)
        except (AgentRunError, OSError):
            self._append_event(run_id, "failed", error_code="RESULT_RECORD_INVALID")
            return True
    ...
    def _execute(self, run_id: str) -> None:
        ...
        try:
            ...
            envelope = _read_file(self._run_root(run_id) / "request.json", max_bytes=1 << 20)
            bundle = verify_review_bundle(envelope, self._public_keys, now=None)
            ...
            self._append_event(run_id, "failed", error_code="AGENT_EXECUTION_FAILED")
    ...
    def _append_event(
        ...
        *,
        ...
        if state not in {"queued", "preparing", "running", "finalizing", *_TERMINAL_STATES}:
            raise ValueError("invalid Agent run state")
        with self._lock:
            ...
            if cached is not None:
                ...
                result_path = self._run_root(run_id) / "result.json"
                if result_path.is_file() and not result_path.is_symlink():
                    updated["result"] = _read_json_object(result_path, max_bytes=2 << 20)
    ...
    def _latest_event(self, run_root: Path) -> dict[str, Any]:
        ...
        if not event_paths:
            raise AgentRunError("Agent run has no state events")
    ...
    def _safe_run_roots(self) -> tuple[Path, ...]:
        return tuple(
            path
            for path in self._root.glob("review-*")
    ...
    def _aware_now(self) -> datetime:
        ...
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("Agent run clock must return a timezone-aware timestamp")
...
def _create_file(path: Path, content: bytes) -> None:
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:
                raise OSError("short write while creating Agent run record")
            view = view[written:]
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _read_file(path: Path, *, max_bytes: int) -> bytes:
    if path.is_symlink() or not path.is_file():
        raise AgentRunError("Agent run record must be a regular file")
    content = path.read_bytes()
    if len(content) > max_bytes:
        raise AgentRunError("Agent run record exceeds its size limit")
    return content
...
def _read_json_object(path: Path, *, max_bytes: int) -> dict[str, Any]:
    try:
        ...
        raise AgentRunError("Agent run record is invalid JSON") from error
    if not isinstance(value, dict):
        raise AgentRunError("Agent run record must be a JSON object")
    return value
