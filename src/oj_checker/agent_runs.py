...
class FileAgentRunService:
    ...
    review into a completed report. There are deliberately no automatic task
    retries; transient model retries remain bounded inside one Agent turn.
    """
    ...
    def latest_many(self, submission_ids: Iterable[str]) -> dict[str, dict[str, Any]]:
        ...
        for run_root in self._safe_run_roots():
            try:
                ...
                continue
            stored_submission_id = metadata.get("submission_id")
            if not isinstance(stored_submission_id, str) or stored_submission_id not in requested:
                continue
            ...
            current = candidates.get(stored_submission_id)
            if current is None or candidate > current:
                candidates[stored_submission_id] = candidate
        results: dict[str, dict[str, Any]] = {}
        for submission_id, (_, run_id) in candidates.items():
            try:
                results[submission_id] = self.get(run_id)
            except (AgentRunError, LookupError, OSError):
                continue
        return results
